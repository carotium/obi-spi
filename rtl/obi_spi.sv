module obi_spi #(
  ...
  **************************************************************/
  ...
  localparam int CtrlCompleteBit             = 3;
  ...
  **************************************************************/
  ...
  typedef enum reg [1:0] {
    ...
    eOBI_TRANSFER    // OBI read/write transfer
  } obi_state_t;
  ...
  **************************************************************/
  ...
  logic [SpiDataLength-1:0] tx_data_reg;
  logic [SpiDataLength-1:0] rx_data_reg;
  logic [DataWidth-1 : 0] spi_div_clk_reg;
  ...
  logic ctrl_busy_bit;
  //  3 bit: Complete (TX or RX) - "Writable" - sets after SPI transaction completes, waits for clear from CPU - IRQ for CPU
  logic ctrl_complete_bit;

  logic ctrl_complete_bit_next, ctrl_complete_bit_write;

  logic [3:0] ctrl_reg_value;
  ...
  **************************************************************/
  ...
  assign ctrl_reg_value = (
      ({3'b0, ctrl_start_writing_bit} << CtrlStartWritingBit)
    | ({3'b0, ctrl_start_reading_bit} << CtrlStartReadingBit)
    | ({3'b0, ctrl_busy_bit} << CtrlBusyBit)
    | ({3'b0, ctrl_complete_bit} << CtrlCompleteBit)
    | 4'b0
  );
  ...
  **************************************************************/
  ...
  assign spi_mosi_o = (spi_state == eSPI_IDLE || spi_data_index == SpiDataLength || spi_state == eSPI_DONE || spi_state == eSPI_READING) ? 1'b0 : tx_data_reg[SpiDataLength - 1 - {28'b0, spi_data_index}];
  ...
  always_comb begin
    spi_stopped_writing =   spi_state == eSPI_WRITING  && {28'b0, spi_data_index} == SpiDataLength;
    spi_stopped_reading = spi_state == eSPI_READING && {28'b0, spi_data_index} == SpiDataLength;
  ...
  **************************************************************/
  ...
  always_comb begin
    unique case (obi_aaddr_i[6:0])
      ...
      CtrlRegAddrOffset:      obi_read_value = {28'b0, ctrl_reg_value};
  ...
  **************************************************************/
  ...
  assign obi_rvalid_o = (obi_state == eOBI_TRANSFER);

  // OBI FSM conditions for transitions
  always_comb begin
    obi_started_transfer = obi_state == eOBI_IDLE && (obi_a_read || obi_a_write);
    obi_done            = obi_rvalid_o && obi_rready_i;
  end

  // OBI FSM transitions
  always_comb begin
    obi_state_next = obi_started_transfer ? eOBI_TRANSFER : obi_state;
